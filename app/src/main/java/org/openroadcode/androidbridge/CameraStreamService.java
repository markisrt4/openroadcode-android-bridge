...
public final class CameraStreamService extends Service {
    ...
    private void startCameraPipeline() {
        if (checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            fail("Camera permission not granted"); return;
        }
        ...
        while (running.get() && encoder != null) {
            try {
                ...
                if (buffer != null && info.size > 0) {
                    ...
                    byte[] encoded = new byte[info.size]; buffer.get(encoded);
                ...
                if (running.get()) fail("Encoder stopped unexpectedly: " + message(e));
                break;
            }
        }
    }
    ...
    private void captureCodecConfig(MediaFormat format) {
        ...
        if (output.size() > 0) codecConfig = output.toByteArray();
    }

    private void appendCodecBuffer(ByteArrayOutputStream output, ByteBuffer buffer) {
        if (buffer == null) return;
        ByteBuffer duplicate = buffer.duplicate();
        byte[] data = new byte[duplicate.remaining()]; duplicate.get(data);
    ...
    private byte[] toAnnexB(byte[] input) {
        ...
        while (offset + 4 <= input.length) {
            ...
            output.write(input, offset, length); offset += length;
        }
    ...
    private void writeVideoFrame(byte[] frame) {
        synchronized (clientLock) {
            ...
            try { videoOutput.write(frame); videoOutput.flush(); }
    ...
    private void startServer() {
        serverThread = new Thread(() -> {
            try (ServerSocket socket = new ServerSocket()) {
                serverSocket = socket; socket.setReuseAddress(true); socket.bind(new InetSocketAddress("0.0.0.0", PORT));
    ...
    private void handleClient(Socket client) {
        try {
            ...
            if (requestLine.startsWith("GET /status ")) writeStatus(client);
            else if (requestLine.startsWith("GET /video ")) attachVideoClient(client);
    ...
    private void attachVideoClient(Socket client) throws IOException {
        ...
        synchronized (clientLock) {
            ...
            videoClient = client; videoOutput = output; connectedClients++;
    ...
    private void writeStatus(Socket client) throws IOException {
        ...
        try {
            ...
            json.put("encoded_frames", encodedFrames); json.put("client_connected", videoOutput != null);
            json.put("client_connections", connectedClients); json.put("error", errorMessage);
